...
/*   Updated: 2025/07/31 13:24:26 by jortiz-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_ray(t_game *game, t_ray *ray, int x)
{
	...
	ray->ray_x = game->player.dir_x
		+ game->player.plane_x * ray->camera_x;
	ray->ray_y = game->player.dir_y
...
void	ft_calculate_steps_and_sidedist(t_game *game, t_ray *ray)
{
	...
	{
		ray->step_x = -1;
		ray->side_distance_x = (game->player.x - ray->map_x)
			* ray->delta_distance_x;
	}
	else
	{
		ray->step_x = 1;
		ray->side_distance_x = (ray->map_x + 1 - game->player.x)
			* ray->delta_distance_x;
	}
	if (ray->ray_y < 0)
	{
		ray->step_y = -1;
		ray->side_distance_y = (game->player.y - ray->map_y)
			* ray->delta_distance_y;
	}
	else
	{
		ray->step_y = 1;
		ray->side_distance_y = (ray->map_y + 1 - game->player.y)
			* ray->delta_distance_y;
	}
}
